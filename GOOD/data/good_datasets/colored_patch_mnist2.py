...
class CPatchMNIST2(InMemoryDataset):
    r"""
        Like CPatchMNIST, but color patches are just 1 and corresponding to the first superpixel.
    """
    ...
    def __init__(self, root: str, domain: str, shift: str = 'no_shift', mode: str = 'train', transform=None,
        ...
        super(CPatchMNIST2, self).__init__(root, transform, pre_transform, None)

        idx = self.processed_file_names.index('cpatchmnist2_75sp_{}.pt'.format(self.mode))
    ...
    def processed_file_names(self):
        return ['cpatchmnist2_75sp_train.pt', 'cpatchmnist2_75sp_test.pt']
        ...
        for index, sample in enumerate(self.sp_data):
            ...
            data_list.append(
                Data(
                    ...
                    name=f'CPatchMNIST2SP-{self.mode}-{index}', idx=index
                )
            )
        idx = self.processed_file_names.index('cpatchmnist2_75sp_{}.pt'.format(self.mode))
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False, debias: bool =False, model_name:str=None, add_pos_feat=False):
        ...
        train_set = CPatchMNIST2(dataset_root + "/CPatchMNIST2/", domain=domain, mode="train")
        test_set = CPatchMNIST2(dataset_root + "/CPatchMNIST2/", domain=domain, mode="test")
